...
export class RezervacijeComponent implements OnInit {
  ...
  constructor(
    protected rezervacijeService: RezervacijeService,
  ...
  trackId(_index: number, item: IRezervacije): number {
    return item.id!;
  }

  delete(rezervacije: IRezervacije): void {
    const modalRef = this.modalService.open(RezervacijeDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.rezervacije = rezervacije;
  ...
  protected onSuccess(data: IRezervacije[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
